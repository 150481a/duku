...
package org.duku.mall.framework.starter.cache;
...
import org.duku.mall.framework.starter.cache.core.CacheGetFilter;
import org.duku.mall.framework.starter.cache.core.CacheGetIfAbsent;
import org.duku.mall.framework.starter.cache.core.CacheLoader;
import org.duku.mall.framework.starter.cache.toolkit.CacheUtil;
import org.duku.mall.framework.starter.cache.toolkit.FastJson2Util;
...
public class StringRedisTemplateProxy implements DistributedCache {
    ...
    private static final String LUA_QUANTITY_SCRIPT_PATH = "lua/quantity.lua";

    private static final String LUA_LIMIT_SCRIPT_PATH = "lua/limit.lua";

    public StringRedisTemplateProxy(StringRedisTemplate stringRedisTemplate, org.duku.mall.framework.starter.cache.config.RedisDistributedProperties redisDistributedProperties, RedissonClient redissonClient, StringRedisTemplate stringRedisTemplate1, RedisDistributedProperties redisProperties, RedissonClient redissonClient1) {
    ...
    public DefaultRedisScript<Boolean> quantityScript() {
        DefaultRedisScript<Boolean> actual = Singleton.get(LUA_QUANTITY_SCRIPT_PATH, () -> {
            DefaultRedisScript redisScript = new DefaultRedisScript();
            redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource(LUA_QUANTITY_SCRIPT_PATH)));
            redisScript.setResultType(Boolean.class);
            return redisScript;
        });
        return actual;
    ...
    public DefaultRedisScript<Long> limitScript() {
        DefaultRedisScript<Long> actual = Singleton.get(LUA_LIMIT_SCRIPT_PATH, () -> {
            DefaultRedisScript redisScript = new DefaultRedisScript();
            redisScript.setScriptSource(new ResourceScriptSource(new ClassPathResource(LUA_LIMIT_SCRIPT_PATH)));
